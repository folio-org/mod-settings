...
public class ConfigStorage {
  ...
  public Future<Void> init() {
    return pool.execute(List.of(
        CREATE_IF_NO_EXISTS + configTable
            ...
            + ")",
        "CREATE UNIQUE INDEX IF NOT EXISTS config_scope_key_userid ON "
            + configTable + "(scope, key, userId)",
        "CREATE INDEX IF NOT EXISTS config_scope ON "
            + configTable + "(scope)"
    ));
  }
  ...
  public Future<Void> deleteEntry(UUID id) {
    return getEntryWoCheck(id).compose(entry -> {
      ...
      return pool.preparedQuery(
              "DELETE FROM " + configTable + " WHERE id = $1")
          .execute(Tuple.of(id))
          .map(res -> {
            if (res.rowCount() == 0) {
              throw new NotFoundException();
    ...
    return pool.preparedQuery(
        ...
        .recover(e -> {
          if (e instanceof PgException pgException) {
            if (pgException.getMessage().contains("\"config_scope_key_userid\" (23505)")) {
